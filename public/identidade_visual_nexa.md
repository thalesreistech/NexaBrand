...
- **Rosa Neon (Rosa Vibrante):** `#FF00FF`
- **Roxo Base (Transição):** `#8A2BE2`
- **Ciano/Azul (Ciano Elétrico):** `#00FFFF`
- **Fundo Oficial Escuro (Dark Mode):** `#0B0E14`
- **Fundo Oficial Claro (Light Mode):** `#FFFFFF`

### Regra de Degradê (Gradient Rule)
O degradê oficial deve seguir a ordem de transição linear de **45 graus** ou **horizontal**:
`linear-gradient(45deg, #FF00FF 0%, #8A2BE2 50%, #00FFFF 100%)`

Use o degradê para:
1. Destaque em títulos importantes (efeito de gradiente no texto).
